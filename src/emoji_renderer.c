...
EmojiRenderer* emoji_renderer_create(SDL_Renderer* ren) {
    ...
    if (er->num_defined_emojis > 0) {
        er->emoji_codepoints_shuffled = (const char**)malloc(sizeof(char*) * er->num_defined_emojis);
        ...
        er->emoji_textures = (SDL_Texture**)calloc(er->num_defined_emojis, sizeof(SDL_Texture*));
        er->emoji_texture_dims = (SDL_Point*)calloc(er->num_defined_emojis, sizeof(SDL_Point));
...
void emoji_renderer_shuffle_and_render_all(EmojiRenderer* er) {
    if (!er || !er->emoji_font || !er->ren_ref || er->num_defined_emojis == 0 || !er->emoji_codepoints_shuffled) {
        return;
    }
    ...
    for (int i = 0; i < er->num_defined_emojis; ++i) {
        if (!er->emoji_codepoints_shuffled[i] || strlen(er->emoji_codepoints_shuffled[i]) == 0) {
            er->emoji_textures[i] = NULL;
            er->emoji_texture_dims[i] = (SDL_Point){0,0};
            continue;
        }
        SDL_Surface *surface = TTF_RenderUTF8_Blended(er->emoji_font, er->emoji_codepoints_shuffled[i], fg_color);
        if (!surface) {
            SDL_Log("Failed to render emoji '%s': %s", er->emoji_codepoints_shuffled[i], TTF_GetError());
            er->emoji_textures[i] = NULL;
            er->emoji_texture_dims[i] = (SDL_Point){0,0};
            continue;
        }
        ...
        if (!er->emoji_textures[i]) {
            SDL_Log("Failed to create texture for emoji '%s': %s", er->emoji_codepoints_shuffled[i], SDL_GetError());
            er->emoji_texture_dims[i] = (SDL_Point){0,0};
        } else {
...
SDL_bool emoji_renderer_get_texture_info(const EmojiRenderer* er, int emoji_array_idx, SDL_Texture** tex, int* w, int* h) {
    if (!er || !tex || !w || !h || emoji_array_idx < 0 || emoji_array_idx >= er->num_defined_emojis) {
        return SDL_FALSE;
    }
