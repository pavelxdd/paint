                  ...
                  SDL_bool        show_emojis)
{
    ...
    if (show_colors && show_emojis && p->emoji_rows > 0 && p->color_rows > 0 && COLOR_EMOJI_SEPARATOR_HEIGHT > 0) {
    ...
    if (show_emojis) {
        ...
        for (int er = 0; er < p->emoji_rows; ++er) {
            ...
            for (int c = 0; c < p->cols; ++c) {
                ...
                SDL_SetRenderDrawColor(ren, (er + c) % 2 == 0 ? chk1.r : chk2.r, (er + c) % 2 == 0 ? chk1.g : chk2.g, (er + c) % 2 == 0 ? chk1.b : chk2.b, 255);
                SDL_RenderFillRect(ren, &cell_r);
                ...
                if (num_available_emojis > 0) {
                    ...
                    if (emoji_renderer_get_texture_info(p->emoji_renderer_instance, actual_idx, &tex, &tex_w, &tex_h) && tex) {
                        ...
                        SDL_Rect dst_r = { cell_r.x + (cell_r.w - def_w) / 2, cell_r.y + (cell_r.h - def_h) / 2, def_w, def_h };
